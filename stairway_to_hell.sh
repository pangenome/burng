...
odgi paths -i $prefix.consensus@10000__y_0_1000000.og -L \
    ...
    | grep -f <(cat consensus_path_names.touched.txt; odgi paths -i $prefix.consensus@10000__y_0_1000000.og -L \
                    | grep ^Link | grep -n -o -Ff <(<consensus_path_names.depth.10_500.tsv | cut -f 1) \
